...
#
# Add support for SCAN Card 2014:
# Danneaugroup
...
# added support for multiple channels <start>

    def set_active_channel(self, chanNo = 1):
        if chanNo <1 or chanNo >10:
            logging.error('Unexpected channel number')
        else:
            logging.debug('Set Keithley 2010 active channel to %i' % chanNo)
            self._visainstrument.write(':rout:close (@%i)' % chanNo)
            self._activechan = chanNo
            chantemp = self.get_active_channel()
            if chantemp != self._activechan:
                logging.error('Set Keithley 2010 active channel failed!!!')
            qt.msleep(0.005)

# added support for multiple channels <end>
...
# added support for multiple channels <start>
    ...
    def get_active_channel(self):
        m_obj = re.search(r"(\d+)", self._visainstrument.ask(':rout:close:stat?'))
        actchantemp = int(m_obj.group(1))
        if actchantemp != self._activechan:
            logging.warn('Active channel incorrect, plz check. actchantemp = %i, self._activechan = %i' % (actchantemp, self._activechan))
        self._activechan = actchantemp 
        return self._activechan
...
# added support for multiple channels <end>
    ...
    def do_get_readval(self, ignore_error=False):
        ...
        if trigger_status:
            if ignore_error:
                logging.debug('Trigger=continuous, can\'t trigger, return 0, plz use readlast function for CONT trigger.')
            else:
                logging.error('Trigger=continuous, can\'t trigger, return 0, plz use readlast function for CONT trigger.')
            text = '0'
            return float(text[0:15])
        elif not trigger_status:
